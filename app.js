...
// let func = async (average_type, smoothe) => {
  ...
  // let data = ["GHSI", "GPL", "GSV", "SYN", "TRX", "ACST", "ITP"];
  let d = new Date();
  ...
  var interval = setAsyncInterval(async func2 => {
    ...
    try {
      console.log('in try',`https://api.tiingo.com/tiingo/daily////${data[iterator]}/prices?token=`)
      ...
      console.log(json1)
      if (json1.length > 0) {
        console.log('inside fetch if',`https://api.tiingo.com/tiingo/daily////${data[iterator]}/prices?startDate=${oldDate}&endDate=${json1[0].date}&token=`)
        //second one
